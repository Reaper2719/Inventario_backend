...
class AmbienteSerializer(serializers.ModelSerializer):
    # Campo para escritura (mapea a sede_nombre en el modelo)
    ...
    def get_centro_nombre(self, obj):
        """Lógica para obtener el nombre del centro relacionado"""
        return "Centro Principal"  # Reemplaza con tu lógica real

    def validate_sede(self, value):
        """Validación para el campo sede"""
        value = value.strip()
        if not value:
...
class RegistroEnergeticoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'subcategoria_2', 'subcategoria_3', 'tipo_aire', 'refrigerante',
        ...
        extra_kwargs = {
            ...
            'tipo_aire': {
                'help_text': 'Tipo de aire acondicionado (solo si categoría es aire acondicionado)'
            },
            'refrigerante': {
    ...
    def validate(self, data):
        """Validación condicional para campos de aire acondicionado"""
        categoria = data.get('categoria_base', '').lower()
        
        if 'aire acondicionado' in categoria:
            if not data.get('tipo_aire'):
                raise serializers.ValidationError({
                    'tipo_aire': 'Este campo es requerido para aire acondicionado'
                })
            ...
            if 'tipo_aire' in data:
                data.pop('tipo_aire')
            if 'refrigerante' in data:
                data.pop('refrigerante')
