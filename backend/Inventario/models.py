...
class RegistroEnergetico(models.Model):
    ...
    tipo_aire = models.CharField(
        max_length=50,
        verbose_name="Tipo de Aire Acondicionado",
        blank=True,
        null=True,
        choices=[
            ('SPLIT', 'Split'),
    ...
    def clean(self):
        """Validación personalizada para campos de aire acondicionado"""
        super().clean()
        if self.categoria_base.lower() == 'aire acondicionado':
            if not self.tipo_aire:
                raise ValidationError({'tipo_aire': 'Debe especificar el tipo de aire acondicionado'})
